...
import { Checkbox, CheckboxGroup, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader, Textarea, useDisclosure } from "@nextui-org/react";
...
export const TaskFeedback:FC<Props> = typedMemo(function TaskFeedback({
    items,
    text,
    className,
}){
    ...
    return (
        <>
            <Button 
                className={clsx(styles.taskFeedback__open, className)}
                variant="faded"
                onClick={onOpen}
                endContent={<img src={Feedback} alt="Send feedback"/>}
            >
                {text}
            </Button>
                    ...
                    {(onClose) => (
                        ...
                        <ModalBody className={styles.taskFeedback__feedback}>
                            <Typography variant="span" className={styles.taskFeedback__aboutFeedback}>
                                Ваши отзывы помогут нам улучшить работу сервиса. Пожалуйста, дайте нам знать, что пошло не так.
                            </Typography>
